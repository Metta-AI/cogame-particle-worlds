import
  std/json,
  mpe/[broadcast, global, replay_runtime, replays, sim]
...
## used to die with mpe_error_len() == 0 and no diagnostic at all). The
...
proc mpeLoadReplay(data: ptr uint8, length: cint): cint
    {.exportc: "mpe_load_replay", cdecl.} =
  try:
    lastError = ""
...
proc mpeInput(data: ptr uint8, length: cint)
    {.exportc: "mpe_input", cdecl.} =
  if runtimeLoaded:
    viewer.applyGlobalViewerMessage(data.bytesFromPointer(int(length)))

proc mpeFrame(): cint {.exportc: "mpe_frame", cdecl.} =
...
proc mpePacketPointer(): ptr uint8
    {.exportc: "mpe_packet_ptr", cdecl.} =
  if packet.len == 0:
    nil
  else:
    packet[0].addr

proc mpePacketLength(): cint {.exportc: "mpe_packet_len", cdecl.} =
  cint(packet.len)

proc mpeMismatchTick(): cint {.exportc: "mpe_mismatch_tick", cdecl.} =
  if runtimeLoaded:
    cint(replay.hashMismatchTick)
  else:
    -1

proc mpeErrorPointer(): ptr uint8 {.exportc: "mpe_error_ptr", cdecl.} =
  if lastError.len == 0:
    nil
  else:
    cast[ptr uint8](lastError[0].addr)

proc mpeErrorLength(): cint {.exportc: "mpe_error_len", cdecl.} =
  cint(lastError.len)

proc mpeStagePointer(): ptr uint8 {.exportc: "mpe_stage_ptr", cdecl.} =
  ## The progress note (see stageNote above). Unlike mpe_error_*, this stays
...
proc mpeStageLength(): cint {.exportc: "mpe_stage_len", cdecl.} =
  cint(stageNoteLen)
...
when isMainModule and defined(emscripten):
  ...
  # module stays alive and JS keeps calling mpe_load_replay/mpe_frame. The
