...
# use-after-free fixed in mpe_replay.nim (emscripten_exit_with_live_runtime)
...
# mpe_error_len(). Aborting keeps linear memory intact, and the page reads
# mpe_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "mpe_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_mpe_load_replay,_mpe_frame,_mpe_input,_mpe_packet_ptr,_mpe_packet_len,_mpe_mismatch_tick,_mpe_error_ptr,_mpe_error_len,_mpe_stage_ptr,_mpe_stage_len
  """).replace("\n", " ")
)
