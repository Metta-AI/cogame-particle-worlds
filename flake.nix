{
  description = "Coworld MPE — Nim capture-the-flag game server, bots, and tooling";
    ...
    in
    {
      packages = forAllSystems (system: pkgs: rec {
        ...
        nimby = pkgs.stdenv.mkDerivation {
          ...
          meta = {
            description = "Nim package manager used by coworld-particle-worlds (nimby.lock)";
      ...
      devShells = forAllSystems (system: pkgs:
        let
          ...
          # rebuild: measured on this repo, `nim c -f src/particle_worlds.nim` goes 15.5s ->
        ...
        in
        {
          default = pkgs.mkShell {
            name = "coworld-particle-worlds";

            packages = [
              # Nim toolchain. nixpkgs-unstable ships 2.2.10, the version the
              # README pins via `nimby use 2.2.10` (particle_worlds.nimble needs >= 2.2.4).
              pkgs.nim
            ...
            shellHook = ''
              if [ -t 1 ]; then
                echo "coworld-particle-worlds dev shell — nim $(nim --version | head -1 | cut -d' ' -f4), node $(node --version), python $(python3 --version | cut -d' ' -f2)"
                if [ ! -f nim.cfg ]; then
                  echo
  ...
  run server   COGAME_HOST=0.0.0.0 COGAME_PORT=2000 \
                 COGAME_CONFIG_URI=file://$PWD/config.json nim r src/particle_worlds.nim
