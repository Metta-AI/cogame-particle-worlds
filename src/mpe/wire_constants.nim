...
## wasm bundle. Clients read `window.MPE_WIRE` and keep their old literals
...
const WireConstantsJs* =
  ...
  "window.MPE_WIRE={speeds:[0.5," & jsIntArray(PlaybackSpeeds)[1..^1] &
  ",fps:" & $TargetFps &
...
const WireConstantsMarker* = "<!-- WIRE_CONSTANTS -->"
  ...
  ## (before any script that reads window.MPE_WIRE).
