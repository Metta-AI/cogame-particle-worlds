...
const
  ...
  HillObjectId = 40770
  # --- particle worlds: the four landmark discs -------------------------
  LandmarkCoverSteps = 11      ## coverage ring fill quantised to 0..10 tenths,
                               ## which bounds the sprite cache at 4 colours x
                               ## 11 steps instead of one bake per permille.
  LandmarkSpriteBase = 35530   ## LandmarkCount x LandmarkCoverSteps keys:
                               ## 35530..35573, in the hill/endzone-fade gap.
  LandmarkObjectBase = 40780   ## one object per mark: 40780..40783, above the
                               ## hill overlay and below the debug pool.
  LandmarkWashPad = 46         ## px the soft radial wash reaches past the
                               ## disc's own radius, so the board itself reads
                               ## as a coverage heatmap.
  LandmarkDiscZ = low(int16) + 4  ## above the floor, below every actor.
  ...
  BoardObjectPools = [
    ...
    ("landmark discs", LandmarkObjectBase, LandmarkCount),
...
const
  ...
  BoardSpritePools = [
    ...
    ("landmark discs", LandmarkSpriteBase, LandmarkCount * LandmarkCoverSteps),
...
proc endzoneStripBox(gameMap: MpeMap, team: Team):
...
proc endzoneShapeToken(gameMap: MpeMap, zone: CaptureZone): string =
...
proc buildSmoothShoutBubble(
  ...
  let cacheKey = $ord(team) & "," & $k & "," & $native & "," &
    $edgeOverride.r & "." & $edgeOverride.g & "." & $edgeOverride.b & "." &
    $edgeOverride.a & "\x1f" & text
  ...
  let
    ...
    edge =
      if edgeOverride.a > 0'u8: edgeOverride
...
proc buildShoutBubble*(
  ...
  if boardScale > 1 or zoom > 1:
    return game.buildSmoothShoutBubble(
      team, text, boardScale * zoom, boardScale, edgeOverride)
  let
    font = game.shoutFont
    ...
    edge =
      if edgeOverride.a > 0'u8: edgeOverride
      else: Palette[teamColor(team) and 0x0f]  # team-colored outline
...
proc symbolBubbleZoom*(game: SimServer): int =
  ## The symbol bubble is drawn at 3x the shout size: one glyph has to be
  ## readable from across a 1235 px board in a 360 px iframe.
  3 * shoutBubbleZoomFor(game.gameMap.width, game.gameMap.height)

proc shoutBubbleMaxHeight*(game: SimServer): int =
  ## The height of the tallest bubble the SERVER can ever produce. Here that is
  ## ONE GLYPH — the radio's whole vocabulary is a single rune — measured in
  ## the font it will be drawn in, at this board's symbol zoom. This is the
  ## RESERVED BAND: sized from the cap the server enforces and measured in the
  ## real font, never by eye, and reserved whether or not anyone is speaking,
  ## so nothing about the scene depends on a symbol landing (the cogchemists
  ## 2026-08-24 rule, which is why --strict-text-bounds can be enabled in CI).
  game.buildShoutBubble(Red, "W", game.symbolBubbleZoom()).height
...
proc addSeatSymbolBubbles*(
  sim: SimServer,
  spriteDefs: var seq[SpriteDefinition],
  currentIds: var seq[int],
  packet: var seq[uint8]
) {.measure.} =
  ## THE RADIO, on the board: one rounded bubble above each particle holding
  ## its current symbol as ONE LARGE GLYPH, tinted with that symbol's fixed
  ## hue (so "F" is the same colour every time anybody says it, in any round).
  ##
  ## Retargeted from the starter's speech-bubble family and using its exact
  ## primitives — `buildShoutBubble` for the art, `shoutBubblePlacement` for
  ## the placement — but with none of the linger/dwell machinery: a symbol
  ## lives for the WHOLE 108-tick turn rather than ShoutTicks, so it is stable
  ## across rendered frames by construction and cannot strobe.
  ##
  ## Silence renders as an EM DASH, never as an empty bubble, so "said
  ## nothing" is visibly a choice. And no text is ever drawn at a negative
  ## coordinate: `shoutBubblePlacement` flips the bubble below the particle
  ## when it does not fit above and clamps both axes into the board rect.
  if sim.config.numAgents <= 0:
    return
  let zoom = sim.symbolBubbleZoom()
  for seat in 0 ..< min(ShoutMaxCount, sim.players.len):
    if seat >= 4:
      break
    ...
    if not player.alive:
      continue
    let
      index = sim.commSymbol[seat]
      text = (if index > 0: symbolText(index) else: "\u2014")
      hue =
        if index > 0: SymbolHues[clamp(index - 1, 0, SymbolHues.high)]
        else: rgba(120, 112, 104, 255)
      bubble = sim.buildShoutBubble(player.team, text, zoom, hue)
      anchor = player.shoutAnchor()
      place = shoutBubblePlacement(
        sim, anchor.x, anchor.tailTipY, bubble.width, bubble.height)
      spriteId = ShoutSpriteBase + seat
      objectId = ShoutObjectBase + seat
    packet.addBoardSpriteChanged(
      spriteDefs,
      spriteId,
      bubble.width,
      ...
      bubble.pixels,
      labelShout(
        teamText(player.team), IdentityNames[sim.cogIdentityIndex(seat)], text),
      native = boardScale
    )
    currentIds.add(objectId)
    packet.addBoardObject(
      objectId, place.x, place.y, ShoutBubbleZ, MapLayerId, spriteId)
...
proc buildLandmarkSprite(
  sim: SimServer, colour, coverStep: int
): tuple[size: int, pixels: seq[uint8]] {.measure.} =
  ## One coloured mark, BAKED: a filled disc of `landmarkRadius`, a 3 px rim,
  ## a soft inner glow, a COVERAGE RING whose fill is `coverStep / 10`, and a
  ## soft radial wash of the same alpha reaching LandmarkWashPad px past the
  ## rim. That wash is the idea's "landmark coverage heatmap": the BOARD reads
  ## as who is covering what, with the HUD off.
  ##
  ## Emitted from the SIM, not drawn by the page, so the overlay is identical
  ## live, in the native replay server and in the wasm bundle — and
  ## broadcast_core.js stays byte-identical to the starter's.
  let
    radius = max(2, sim.config.landmarkRadius)
    pad = LandmarkWashPad
    size = 2 * (radius + pad) + 1
    centre = radius + pad
    tint = MarkColours[clamp(colour, 0, LandmarkCount - 1)]
    fill = clamp(coverStep, 0, LandmarkCoverSteps - 1)
  result.size = size
  result.pixels = newRgbaPixels(size, size)
  for y in 0 ..< size:
    for x in 0 ..< size:
      let
        dx = x - centre
        ...
        i = y * size + x
      if d2 <= radius * radius:
        ## The disc: a bright rim (the outer 3 px) over a body that brightens
        ## toward the middle, so a mark reads as a lit pad rather than a blob.
        let inner = radius - 3
        if d2 > inner * inner:
          result.pixels.putRawRgbaPixel(i, tint.r, tint.g, tint.b, 255)
        else:
          let glow = 150 + 80 * (inner * inner - d2) div max(1, inner * inner)
          result.pixels.putRawRgbaPixel(
            i,
            uint8(min(255, int(tint.r) * glow div 255 + 24)),
            uint8(min(255, int(tint.g) * glow div 255 + 24)),
            uint8(min(255, int(tint.b) * glow div 255 + 24)),
            uint8(min(255, 210)))
        continue
      if fill == 0:
        continue
      ## The coverage ring: an annulus just outside the disc whose RADIAL
      ## thickness is the coverage fraction.
      let
        ringOuter = radius + 7
        thickness = max(1, 7 * fill div (LandmarkCoverSteps - 1))
        ringInner = ringOuter - thickness
      if d2 <= ringOuter * ringOuter and d2 > ringInner * ringInner:
        result.pixels.putRawRgbaPixel(i, tint.r, tint.g, tint.b, 235'u8)
        continue
      ## The soft wash: alpha falls to nothing at the pad edge and scales with
      ## the same coverage fraction.
      let outer = radius + pad
      if d2 <= outer * outer:
        let
          span = max(1, outer - radius)
          here = max(0, outer - (radius + (d2 - radius * radius) div
            max(1, 2 * radius)))
          falloff = clamp(here * 100 div span, 0, 100)
          alpha = 78 * fill div (LandmarkCoverSteps - 1) * falloff div 100
        if alpha > 0:
          result.pixels.putRawRgbaPixel(
            i, tint.r, tint.g, tint.b, uint8(clamp(alpha, 0, 255)))

proc landmarkSpriteSize*(sim: SimServer): int {.inline.} =
  ## The square side of one baked mark sprite: the disc plus the soft wash.
  2 * (max(2, sim.config.landmarkRadius) + LandmarkWashPad) + 1

proc addSeatLandmarks(
  sim: SimServer,
  spriteDefs: var seq[SpriteDefinition],
  currentIds: var seq[int],
  packet: var seq[uint8]
) {.measure.} =
  ## The four marks. NEVER solid — a particle glides over one in every mode —
  ## and never a sprite that occupies an actor slot: the object pools stay at
  ## four moving bodies plus these four static discs.
  if sim.config.numAgents <= 0:
    return
  for i in 0 ..< sim.landmarks.len:
    var nearest = high(int)
    for seat in 0 ..< min(4, sim.players.len):
      nearest = min(nearest, sim.distanceTo(seat, i))
    let
      close =
        if nearest == high(int): 0
        else: closeness(nearest, sim.config.closeScalePx)
      step = clamp(close * (LandmarkCoverSteps - 1) div 1000,
                   0, LandmarkCoverSteps - 1)
      colour = clamp(sim.landmarks[i].colour, 0, LandmarkCount - 1)
      spriteId = LandmarkSpriteBase + colour * LandmarkCoverSteps + step
      objectId = LandmarkObjectBase + i
      span = sim.landmarkSpriteSize()
    ## BAKE ONCE PER KEY. There are only LandmarkCount x LandmarkCoverSteps = 44
    ## distinct disc sprites, and each is ~66 KB of RGBA at 1x; re-sending the
    ## pixels every frame put 6.3 MB of images on a 40 s episode's four seat
    ## streams and made the server frame-bound (measured). The definition is
    ## sent the first time this viewer sees the id and the OBJECT alone moves
    ## afterwards, exactly as the rig segments do.
    if spriteDefs.spriteDefinitionIndex(spriteId) < 0:
      let art = sim.buildLandmarkSprite(colour, step)
      packet.addBoardSpriteChanged(
        spriteDefs,
        spriteId,
        art.size,
        art.size,
        art.pixels,
        "mark " & MarkColourNames[colour] & " " & $(step * 10) & "%",
        native = boardScale
      )
    currentIds.add(objectId)
    packet.addBoardObject(
      objectId,
      sim.landmarks[i].x - span div 2,
      sim.landmarks[i].y - span div 2,
      LandmarkDiscZ,
      MapLayerId,
      spriteId
    )
...
proc buildSpriteProtocolPlayerUpdates*(
  ...
  if sim.phase != Playing or playerIndex < 0 or
    ...
    if not viewerIsGhost and not spritesOff and not sim.config.fullyObservable:
    ...
    ## All four marks, in every mode, always: they are the geometry of the
    ## game and every seat is entitled to them.
    ...
    ## The RADIO is global: every seat hears every symbol whatever the
    ## distance, so a seat frame carries the same four bubbles the board does.
    sim.addSeatSymbolBubbles(nextState.spriteDefs, currentIds, result)
    if not viewerIsGhost:
