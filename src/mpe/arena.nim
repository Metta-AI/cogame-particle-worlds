...
## (selectMpeMap/loadMpeMap — including the import-time default-arena
...
proc validateMapRect(name: string, rect: MapRect, width, height: int) =
  ...
  if rect.w <= 0 or rect.h <= 0:
    raise newException(MpeError, "Map " & name & " size must be positive.")
  if rect.x < 0 or rect.y < 0 or
      rect.x + rect.w > width or rect.y + rect.h > height:
    raise newException(MpeError, "Map " & name & " is outside the map.")
...
proc validateMap(gameMap: MpeMap) =
  ## Raises if a loaded map has invalid geometry.
  if gameMap.width <= 0 or gameMap.height <= 0:
    raise newException(MpeError, "Map dimensions must be positive.")
  case gameMap.layout
  of layoutSides:
    if gameMap.symmetry in {symRot90, symQuadMirror}:
      raise newException(
        MpeError, "Sides maps cannot use a 4-team symmetry (rot90/quadmirror).")
  of layoutCorners, layoutPlus:
    if gameMap.symmetry notin {symRot90, symQuadMirror}:
      raise newException(
        MpeError, "Corner/plus maps need rot90 or quadmirror symmetry.")
  ...
  ## coworld-particle-worlds#280 full-board (symNone) invariants. Fairness is NOT checked
...
const
  ...
  FieldMapName* = "field"      ## the particle-worlds board: border walls only.
...
proc homeDepthOf(gameMap: MpeMap): int =
...
proc rot90Quarter*(gameMap: MpeMap, team: Team): int =
...
proc rot90TeamPoint*(gameMap: MpeMap, red: MapPoint, team: Team): MapPoint =
...
proc teamImagePoint*(gameMap: MpeMap, red: MapPoint, team: Team): MapPoint =
...
proc homeSlot*(gameMap: MpeMap, team: Team): Team =
...
proc slotAnchor*(gameMap: MpeMap, slot: Team): MapPoint =
...
proc teamAnchor*(gameMap: MpeMap, team: Team): MapPoint =
...
proc spawnPocketHalf*(gameMap: MpeMap, team: Team): tuple[w, h: int] =
...
proc plusArmHalf*(gameMap: MpeMap): int =
...
proc plusArmBandOn*(gameMap: MpeMap, span: int): tuple[lo, hi: int] =
...
proc defaultMpeRooms(gameMap: MpeMap): seq[Room] =
...
proc arenaMpeMap(): MpeMap =
  ...
  result.rooms = result.defaultMpeRooms()
  result.validateMap()

proc arenaLargeMpeMap(): MpeMap =
...
proc fieldMpeMap(): MpeMap =
  ## THE particle-worlds board. Hand-authored, 1235 x 659 map pixels — the
  ## starter `arena`'s dimensions on purpose, so every dimension-coupled
  ## constant (ShoutRange = MapWidth div 5, boardRenderScaleFor, the FOV grid,
  ## relayout()'s fit) is inherited unchanged and untested-drift-free.
  ##
  ## BORDER WALLS ONLY: `leftObstacles` is empty, there are no trenches, no
  ## puddles and no animated diamonds, so the wall mask is the perimeter and
  ## nothing else. MPE's plane is unbounded with a soft out-of-bounds penalty;
  ## this field is HARD-BOUNDED by walls, which keeps every particle on screen
  ## (the replay requires it) and makes `evade` a real skill instead of a
  ## straight line to infinity.
  ##
  ## Four team anchors, one per corner, so `teamAnchor` / `captureZone` /
  ## `flagHome` stay defined for the inherited code that reads them. A corner
  ## layout needs a 4-team symmetry and the board is a rectangle, so the
  ## symmetry is quad-mirror (rot90 needs a square).
  result.name = FieldMapName
  result.path = FieldMapName
  result.width = 1235
  result.height = 659
  result.mapLayer = 0
  result.walkLayer = 1
  ...
  result.spawnClearH = 130
  result.gunRange = GunRange
  result.layout = layoutCorners
  result.symmetry = symQuadMirror
...
proc captureZone*(gameMap: MpeMap, team: Team): CaptureZone =
...
proc `==`*(a, b: ArenaShape): bool =
  ...
  ## MpeMap values compare, which the map-spec round-trip tests rely on.
...
proc symmetryImages*(gameMap: MpeMap, rect: MapRect): seq[MapRect] =
...
proc symmetryImages*(gameMap: MpeMap, point: MapPoint): seq[MapPoint] =
...
proc buildArenaObstacles*(gameMap: MpeMap): seq[ArenaShape] =
  ...
  ##
  ## Under symNone (coworld-particle-worlds#280) there is no fundamental domain: the
...
proc isSpinningDiamond*(gameMap: MpeMap, shape: ArenaShape): bool {.inline.} =
...
proc buildAnimatedDiamonds*(
  gameMap: MpeMap, obstacles: seq[ArenaShape]
...
proc centerPuddleSplat(rng: var MapRng, gameMap: MpeMap): Puddle =
...
proc mapSizeScale(sizeName: string): float =
  ...
  of "colossal": 5.2  ## override-only (not in MapSizeNames): 2x giant.
    ...
    raise newException(MpeError, "Unknown map size: " & sizeName)

proc scaledGenShell(sizeName: string): MpeMap =
...
proc mapProtectedFloorAt*(gameMap: MpeMap, x, y: int): bool =
...
proc validateMapWalkability*(gameMap: MpeMap) =
  ## coworld-particle-worlds#280 WALL-OVERLAP check for symNone explicit pickups. Separate
...
proc scaledGenShell4(sizeName: string): MpeMap =
...
proc sightlineLoX*(gameMap: MpeMap): int =
...
proc sightlineHiX*(gameMap: MpeMap): int =
...
proc sightlineLoY*(gameMap: MpeMap): int =
...
proc sightlineHiY*(gameMap: MpeMap): int =
...
proc rectOnOpenFloor(
  gameMap: MpeMap, obstacles: seq[ArenaShape], rect: MapRect
): bool =
...
proc placePuddles(gameMap: var MpeMap, count: int, rng: var MapRng) =
  ...
  for room in gameMap.defaultMpeRooms():
    if room.name != "Center":
  ...
  proc addPuddlePair(
    gameMap: MpeMap, splats: var seq[Puddle], splat: Puddle
  ): bool =
...
proc placePuddles*(gameMap: var MpeMap, count: int, seed: int) =
  ...
  if count > MaxPuddles:
    raise newException(
      MpeError, "Config field mapPuddles must be 0.." & $MaxPuddles & ".")
  if count > 0 and gameMap.symmetry in {symRot90, symQuadMirror}:
    raise newException(
      MpeError, "Puddles are not supported on 4-team maps yet.")
  gameMap.puddles = @[]
...
proc generateMapAttempt*(
...
): MpeMap =  ## One UNVALIDATED draw. Every top-level parameter is drawn unconditionally
  ...
  if teams == 4:
    ...
    if overrides.symmetry notin ["", "rot90", "quadmirror"]:
      raise newException(
        MpeError, "4-team maps are rot90 or quadmirror; got mapSymmetry: " &
          overrides.symmetry)
    ...
    result.layout =
      ...
      of "plus": layoutPlus
        ...
        raise newException(
          MpeError, "Unknown map layout: " & overrides.layout)
  else:
    ...
    result.symmetry =
      ...
      of "rot180": symRot180
        ...
        raise newException(
          MpeError, "Unknown map symmetry: " & overrides.symmetry)
    if overrides.layout.len > 0 and overrides.layout != "sides":
      raise newException(
        MpeError, "Map layout " & overrides.layout & " needs teams: 4.")
  ...
  block endzoneDraw:
    ...
    result.endzone =
      ...
      of "square": ezSquare
        ...
        raise newException(
          MpeError, "Unknown map endzone: " & overrides.endzone)
    ...
    if teams == 4:
      raise newException(
        MpeError, "Compact endzones (mapEndzone) need a 2-team map.")
    ...
    if result.homeDepth < HomeDepthMin or result.homeDepth > HomeDepthMax:
      raise newException(
        MpeError, "Config field mapBaseDepth must be " & $HomeDepthMin &
          ".." & $HomeDepthMax & ".")
    if result.endzoneRadius < EndzoneRadiusMin or
        result.endzoneRadius > maxEndzoneRadius(result.width):
      raise newException(
        MpeError, "Config field mapEndzoneRadius must be " &
  ...
  if feature notin CenterFeatureNames:
    raise newException(MpeError, "Unknown map center feature: " & feature)
  ...
  if columns < 3 or columns > maxColumns:
    raise newException(
      MpeError, "Config field mapColumns must be 3.." & $maxColumns & ".")
  ...
  if overrides.pits < -1 or overrides.pits > 64:
    raise newException(MpeError, "Config field mapPits must be 0..64.")
  if overrides.puddles > MaxPuddles:
    raise newException(
      MpeError, "Config field mapPuddles must be 0.." & $MaxPuddles & ".")
  if overrides.pitDensity < -1 or overrides.pitDensity > 1000:
    raise newException(
      MpeError, "Config field mapPitDensity must be 0..1000.")
  let
  ...
  if result.symmetry in {symRot90, symQuadMirror}:
    ...
    if overrides.pits > 0:
      raise newException(
        MpeError, "Trenches are not supported on 4-team maps yet.")
  ...
  block sightlineRepair:
    proc rowBlocked(gameMap: MpeMap, y: int): bool =
      for x in gameMap.sightlineLoX .. gameMap.center.x:
        if mapWallAt(gameMap, gameMap.leftObstacles, x, y):
          return true
      false
    proc rowBlockedFull(gameMap: MpeMap, obstacles: seq[ArenaShape],
        y: int): bool =
  ...
  block columnSightlineRepair:
    ...
    proc colBlocked(gameMap: MpeMap, x: int): bool =
      ...
      for y in gameMap.sightlineLoY .. gameMap.sightlineHiY:
        if mapWallAt(gameMap, gameMap.leftObstacles, x, y,
          ...
          return true
      false
    proc colFullyProtected(gameMap: MpeMap, x: int): bool =
  ...
  if windowCount > 6:
    raise newException(MpeError, "Config field mapWindows must be 0..6.")
  ...
  block finalizeTrenches:
    ...
    if oddCenterPit:
      ...
      digs.add centerPit
    proc addPair(
      gameMap: MpeMap, digs: var seq[MapRect], trench: MapRect
    ): bool =
...
proc validateGeneratedMap*(gameMap: MpeMap): string =
...
proc poolMpeMap*(
  ...
  generateMpeMap(MapPoolSeeds[((index mod n) + n) mod n], overrides)
...
proc shapeFromSpecNode(node: JsonNode): ArenaShape =
  ...
  of "polygon":
    ...
    raise newException(
      MpeError, "Unknown map spec shape: " & node["kind"].getStr())
...
proc mapSpecJson*(gameMap: MpeMap): string =
...
proc mapFromSpecJson*(text: string): MpeMap =
  ...
  try:
    ...
    raise newException(MpeError, "Could not parse map spec JSON: " & e.msg)
  ...
  result.symmetry =
    ...
    of "none": symNone          # coworld-particle-worlds#280 full-board authoring (no lift)
    else:
      raise newException(
        MpeError, "Unknown map spec symmetry: " & symmetryText)
  ...
  result.layout =
    ...
    of "plus": layoutPlus
      ...
      raise newException(MpeError, "Unknown map spec layout: " & layoutText)
  ...
  result.endzone =
    ...
    of "square": ezSquare
      ...
      raise newException(MpeError, "Unknown map spec endzone: " & endzoneText)
  ...
  ## Optional: EXPLICIT per-team pickup points (coworld-particle-worlds#280). Only symNone
...
proc resolveMpeMapMetadata*(config: GameConfig): MpeMap =
  ...
  result =
    if config.mapSpec.len > 0:
      ...
      case name
      of ArenaName: arenaMpeMap()
      of ArenaLargeName: arenaLargeMpeMap()
      of FieldMapName: fieldMpeMap()
      of GenMapName: generateMpeMap(genSeed, config.mapGen, config.teams)
      of PoolMapName:
        if config.teams != 2:
          raise newException(
            MpeError, "The curated pool is 2-team; use mapPath gen for " &
              $config.teams & " teams.")
        let index =
          if config.mapPoolIndex >= 0: config.mapPoolIndex else: genSeed
        poolMpeMap(index, config.mapGen)
      else:
        raise newException(MpeError, "Unknown map: " & name)
  if result.teamCount() != config.teams:
    raise newException(
      MpeError, "Config asks for " & $config.teams & " teams but map " &
...
## The SELECTED map's layout, installed once per process by loadMpeMap and
...
proc selectMpeMap(gameMap: MpeMap) =
...
proc installDefaultArena*() =
  ...
  ## import-time `selectMpeMap(arenaMpeMap())` side effect with this explicit
  ## call: constructing any sim (initSimServer -> loadMpeMap) installs its
  ...
  selectMpeMap(arenaMpeMap())

proc loadMpeMapMetadata*(path = ""): MpeMap =
...
proc loadMpeMap*(path = ""): MpeMap =
  ...
  result = loadMpeMapMetadata(path)
  selectMpeMap(result)

proc loadMpeMap*(config: GameConfig): MpeMap =
  ## Resolves the config's effective map and installs it as this process's
  ## arena.
  result = resolveMpeMapMetadata(config)
  selectMpeMap(result)
...
proc arenaCenterOffset2(x, y, cx, cy: int): tuple[dx, dy: int] {.inline.} =
  ## The installed-map twin of MpeMap.centerOffset2: twice the offset from
...
proc mapProtectedFloorAtF*(
  gameMap: MpeMap, x, y: float, cx, cy: int
): bool =
