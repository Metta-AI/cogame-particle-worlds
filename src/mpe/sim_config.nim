...
proc defaultGameConfig*(): GameConfig =
  ## Returns the default MPE gameplay config.
  GameConfig(
    ...
    pursuerSpeedPct: DefaultPursuerSpeedPct,
    landmarkRadius: DefaultLandmarkRadius,
    landmarkMargin: DefaultLandmarkMargin,
    landmarkSpacingPx: DefaultLandmarkSpacingPx,
    spawnRingPx: DefaultSpawnRingPx,
    closeScalePx: DefaultCloseScalePx,
    bumpPx: DefaultBumpPx,
    bumpPenaltyPermille: DefaultBumpPenaltyPermille,
    bumpPenaltyCap: DefaultBumpPenaltyCap,
    tagPx: DefaultTagPx,
    tagTargetTicks: DefaultTagTargetTicks,
    orbitRadiusPx: DefaultOrbitRadiusPx,
    shadowStandoffPx: DefaultShadowStandoffPx,
    evadeProbePx: DefaultEvadeProbePx,
...
proc readConfigInt(node: JsonNode, name: string, value: var int) =
  ...
  let item = node[name]
  if item.kind != JInt:
    raise newException(MpeError, "Config field " & name & " must be an integer.")
  value = item.getInt()
...
proc readConfigBool(node: JsonNode, name: string, value: var bool) =
  ...
  let item = node[name]
  if item.kind != JBool:
    raise newException(MpeError, "Config field " & name & " must be a boolean.")
  value = item.getBool()
...
proc readConfigString(node: JsonNode, name: string, value: var string) =
  ...
  if item.kind != JString:
    raise newException(MpeError, "Config field " & name & " must be a string.")
  value = item.getStr()
...
proc readConfigSlots(node: JsonNode, slots: var seq[PlayerSlotConfig]) =
  ...
  if items.kind != JArray:
    raise newException(MpeError, "Config field slots must be an array.")
  slots.setLen(0)
...
proc readConfigPlayers(node: JsonNode, slots: var seq[PlayerSlotConfig]) =
  ...
  if items.kind != JArray:
    raise newException(MpeError, "Config field players must be an array.")
...
proc readConfigTokens(
  ...
  if items.kind != JArray:
    raise newException(MpeError, "Config field tokens must be an array.")
  if items.len > MaxPlayers:
    raise newException(
      MpeError,
...
proc readConfigPerks(node: JsonNode, config: var GameConfig) =
  ...
  ##     unnamed per-policy groups dealt in join order (MPE-Doubles);
  ...
  if perks.kind != JObject:
    raise newException(MpeError, "Config field perks must be an object.")
...
proc readConfigPerkMods(node: JsonNode, config: var GameConfig) =
  ...
  if mods.kind != JObject:
    raise newException(MpeError, "Config field perkMods must be an object.")
  for key in mods.keys:
    if key notin ["armorHp", "scopeAim", "grenadeRange", "thrusterSpeed",
        "luckChance", "luckDamage"]:
      raise newException(
        MpeError, "Config field perkMods has unknown key " & key & ".")
  ...
  if config.perkMods.luckDamage < 1:
    raise newException(
      MpeError, "Config field perkMods.luckDamage must be at least 1.")
...
proc readConfigHandicaps(node: JsonNode, config: var GameConfig) =
  ...
  if handicaps.kind != JObject:
    raise newException(MpeError, "Config field handicaps must be an object.")
...
proc readConfigRegimes(node: JsonNode, config: var GameConfig) =
  ...
  if items.kind != JArray:
    raise newException(MpeError, "Config field regimes must be an array.")
  if items.len < 1 or items.len > 4:
    raise newException(
      MpeError, "Config field regimes must have 1..4 entries.")
...
proc roundsText*(config: GameConfig): string =
  ## The mode sequence as a readable, comma-joined string, for the startup
  ## echo and the game log.
  for i, mode in config.rounds:
    if i > 0:
      result.add(",")
    result.add($mode)

proc parseMode*(text: string): Mode =
  ## Parses one authored round mode; raises MpeError on an unknown name. A
  ## CLOSED enum: an unrecognised mode is a config error, never a silent
  ## default, because the mode decides who is told the round's secret.
  for mode in Mode:
    if $mode == text.strip().toLowerAscii():
      return mode
  raise newException(MpeError, "Unknown round mode: " & text)

proc readRounds(node: JsonNode, config: var GameConfig) =
  ## Reads the `rounds` array: the mode played by each round of the episode.
  if not node.hasKey("rounds"):
    return
  let items = node["rounds"]
  if items.kind != JArray:
    raise newException(MpeError, "Config field rounds must be an array.")
  var rounds: seq[Mode]
  for i in 0 ..< items.len:
    let item = items[i]
    if item.kind != JString:
      raise newException(
        MpeError, "Config field rounds[" & $i & "] must be a string.")
    rounds.add(parseMode(item.getStr()))
  config.rounds = rounds

proc validate(config: GameConfig) =
  ## Raises if a gameplay config has invalid values.
  if config.motionScale <= 0:
    raise newException(MpeError, "Config field motionScale must be positive.")
  if config.frictionDen <= 0:
    raise newException(MpeError, "Config field frictionDen must be positive.")
  if config.minPlayers < 1:
    raise newException(MpeError, "Config field minPlayers must be at least 1.")
  if config.teams notin [2, 4]:
    raise newException(MpeError, "Config field teams must be 2 or 4.")
  ...
  if config.minPlayers > MaxPlayers:
    raise newException(MpeError, "can't do more than 8 players.")
  if config.lives < 1:
    raise newException(MpeError, "Config field lives must be at least 1.")
  if config.hitPoints < 1:
    raise newException(MpeError, "Config field hitPoints must be at least 1.")
  if config.gunRange <= 0:
    raise newException(MpeError, "Config field gunRange must be positive.")
  if config.fireWindupTicks < 0:
    raise newException(MpeError, "Config field fireWindupTicks must not be negative.")
  if config.carrierSpeedPct <= 0 or config.carrierSpeedPct > 100:
    raise newException(MpeError, "Config field carrierSpeedPct must be 1..100.")
  if config.playerBouncePct < 0 or config.playerBouncePct > 100:
    raise newException(MpeError, "Config field playerBouncePct must be 0..100.")
  if config.aimTurnRate < 1:
    raise newException(MpeError, "Config field aimTurnRate must be at least 1.")
  if config.visionConeDeg < 0 or config.visionConeDeg > 180:
    raise newException(MpeError, "Config field visionConeDeg must be between 0 and 180.")
  if config.puddleDamagePct < 0 or config.puddleDamagePct > 100:
    raise newException(MpeError, "Config field puddleDamagePct must be 0..100.")
  if config.barrierPickups < 0 or
      config.barrierPickups > MaxBarrierPickupsPerTeam:
    raise newException(MpeError,
      "Config field barrierPickups must be 0.." & $MaxBarrierPickupsPerTeam & ".")
  if config.visionBubble < 0:
    raise newException(MpeError, "Config field visionBubble must be non-negative.")
  ...
  if config.startWaitTicks < 0:
    raise newException(MpeError, "Config field startWaitTicks must be non-negative.")
  if config.lobbyJoinTimeoutTicks < 0:
    raise newException(MpeError, "Config field lobbyJoinTimeoutTicks must be non-negative.")
  if config.respawnTicks < 0 or config.fireCooldownTicks < 0:
    raise newException(MpeError, "Timer config fields must not be negative.")
  ...
  if config.barrageMaxPerSec > 0:
    ...
    if config.barrageStartSec < 1:
      raise newException(
        MpeError, "Config field barrageStartSec must be at least 1.")
  ...
  if config.paintTile < 4:
    raise newException(MpeError, "Config field paintTile must be at least 4.")
  if config.hillRadiusTiles < 0:
    raise newException(
      MpeError, "Config field hillRadiusTiles must not be negative.")
  ...
  if config.hillDecisiveTicks < 1:
    raise newException(
      MpeError, "Config field hillDecisiveTicks must be at least 1.")
  if config.paintSpeedOwnPct < 1 or config.paintSpeedEnemyPct < 1:
    raise newException(
      MpeError, "Config field paintSpeed*Pct must be positive.")
  if config.paintHealTicks < 1:
    raise newException(
      MpeError, "Config field paintHealTicks must be at least 1.")
  if config.sprayDamage < 1:
    raise newException(
      MpeError, "Config field sprayDamage must be at least 1.")
  if config.regimes.len < 1 or config.regimes.len > 4:
    raise newException(
      MpeError, "Config field regimes must have 1..4 entries.")
  if config.turnTicks < 1:
    raise newException(MpeError, "Config field turnTicks must be at least 1.")
  if config.turnBudgetMs < 1 or config.attempt1Ms < 1 or config.retryMs < 1:
    raise newException(
      MpeError, "Config LLM deadline fields must be positive.")
  ...
  if config.turnSpacingMs < 0:
    raise newException(
      MpeError, "Config field turnSpacingMs must not be negative.")
  if config.wallClockBudgetSeconds < 1:
    raise newException(
      MpeError, "Config field wallClockBudgetSeconds must be positive.")
  if config.paintBuff and not config.floorPaint:
    raise newException(
      MpeError, "Config field paintBuff requires floorPaint.")
  if config.hill and not config.floorPaint:
    raise newException(
      MpeError, "Config field hill requires floorPaint.")
  if config.numAgents < 0:
    raise newException(MpeError, "Config field num_agents must not be negative.")
  # --- particle worlds ---
  if config.rounds.len < 1 or config.rounds.len > 4:
    raise newException(
      MpeError, "Config field rounds must have 1..4 entries.")
  if config.pursuerAccelPct < 1 or config.pursuerAccelPct > 100:
    raise newException(MpeError, "Config field pursuerAccelPct must be 1..100.")
  if config.pursuerSpeedPct < 1 or config.pursuerSpeedPct > 100:
    raise newException(MpeError, "Config field pursuerSpeedPct must be 1..100.")
  if config.landmarkRadius < 1:
    raise newException(
      MpeError, "Config field landmarkRadius must be at least 1.")
  if config.landmarkMargin < config.landmarkRadius:
    raise newException(
      MpeError, "Config field landmarkMargin must be >= landmarkRadius.")
  if config.landmarkSpacingPx < MinLandmarkSpacingPx:
    raise newException(
      MpeError,
  ...
  if config.closeScalePx < 1:
    raise newException(MpeError, "Config field closeScalePx must be positive.")
  if config.bumpPx < 0 or config.tagPx < 0:
    raise newException(
      MpeError, "Config fields bumpPx and tagPx must not be negative.")
  if config.bumpPenaltyPermille < 0 or config.bumpPenaltyPermille > 1000:
    raise newException(
      MpeError, "Config field bumpPenaltyPermille must be 0..1000.")
  if config.bumpPenaltyCap < 0 or config.bumpPenaltyCap > 1000:
    raise newException(
      MpeError, "Config field bumpPenaltyCap must be 0..1000.")
  if config.tagTargetTicks < 1:
    raise newException(
      MpeError, "Config field tagTargetTicks must be at least 1.")
  if config.orbitRadiusPx < 1 or config.shadowStandoffPx < 1 or
      config.evadeProbePx < 1:
    raise newException(
      MpeError,
      "Config fields orbitRadiusPx, shadowStandoffPx and evadeProbePx must " &
        "be positive.")
  if config.symbolCount < 1 or config.symbolCount > SymbolAlphabet.len:
    raise newException(
      MpeError,
      "Config field symbolCount must be 1.." & $SymbolAlphabet.len & ".")
  if config.symbolCount < LandmarkCount:
    raise newException(
      MpeError,
      "Config field symbolCount must be at least " & $LandmarkCount &
        ": the crypto key draws one distinct symbol per mark colour.")
  if config.slots.len > MaxPlayers:
    raise newException(MpeError, "Config field slots cannot have more than 8 entries.")
...
proc update*(config: var GameConfig, jsonText: string) =
  ...
  try:
    ...
    raise newException(MpeError, "Could not parse config JSON: " & e.msg)
  if node.kind != JObject:
    raise newException(MpeError, "Config must be a JSON object.")
  ...
  node.readConfigInt("pursuerSpeedPct", config.pursuerSpeedPct)
  node.readConfigInt("landmarkRadius", config.landmarkRadius)
  node.readConfigInt("landmarkMargin", config.landmarkMargin)
  node.readConfigInt("landmarkSpacingPx", config.landmarkSpacingPx)
  node.readConfigInt("spawnRingPx", config.spawnRingPx)
  node.readConfigInt("closeScalePx", config.closeScalePx)
  node.readConfigInt("bumpPx", config.bumpPx)
  node.readConfigInt("bumpPenaltyPermille", config.bumpPenaltyPermille)
  node.readConfigInt("bumpPenaltyCap", config.bumpPenaltyCap)
  node.readConfigInt("tagPx", config.tagPx)
  node.readConfigInt("tagTargetTicks", config.tagTargetTicks)
  node.readConfigInt("orbitRadiusPx", config.orbitRadiusPx)
  node.readConfigInt("shadowStandoffPx", config.shadowStandoffPx)
  node.readConfigInt("evadeProbePx", config.evadeProbePx)
  node.readConfigInt("symbolCount", config.symbolCount)
  node.readConfigRegimes(config)
  if node.hasKey("mapSpec"):
    if node["mapSpec"].kind != JObject:
      raise newException(MpeError, "Config field mapSpec must be an object.")
  ...
  ## map-wide default.
  let mapMeta = resolveMpeMapMetadata(config)
...
proc configJson*(config: GameConfig): string =
  ...
  if particlesOn:
    ...
    node["rounds"] = (block:
      var arr = newJArray()
      for mode in config.rounds:
        arr.add(%($mode))
      arr)
    node["fullyObservable"] = %config.fullyObservable
    node["pursuerAccelPct"] = %config.pursuerAccelPct
    node["pursuerSpeedPct"] = %config.pursuerSpeedPct
    node["landmarkRadius"] = %config.landmarkRadius
    node["landmarkMargin"] = %config.landmarkMargin
    node["landmarkSpacingPx"] = %config.landmarkSpacingPx
    node["spawnRingPx"] = %config.spawnRingPx
    node["closeScalePx"] = %config.closeScalePx
    node["bumpPx"] = %config.bumpPx
    node["bumpPenaltyPermille"] = %config.bumpPenaltyPermille
    node["bumpPenaltyCap"] = %config.bumpPenaltyCap
    node["tagPx"] = %config.tagPx
    node["tagTargetTicks"] = %config.tagTargetTicks
    node["orbitRadiusPx"] = %config.orbitRadiusPx
