...
import
  std/[json, strutils],
  sim_types, sim_state, field, scoring
...
proc resolvePlayerSlot*(
  ...
  if token.len > 0 and sim.config.hasConfiguredTokens() and
    ...
    raise newException(MpeError, "Player token is not configured.")
  if requestedSlot >= 0:
    if requestedSlot >= sim.config.playerSlotLimit():
      raise newException(MpeError, "Player slot is outside configured roster.")
  ...
  if result < 0:
    raise newException(MpeError, "No available player slot.")
...
proc addPlayer*(
  ...
  if not sim.canAddPlayer():
    raise newException(MpeError, sim.config.playerLimitError())
...
proc particleResultsJson*(sim: SimServer): string =
  ## The particle-worlds results document: ONE entry per SEAT, never per
  ## particle, with exactly the 22 keys the manifest's `results_schema`
  ## declares.
  ##
  ...
  ## `tests/test_manifest.nim` asserts the two agree in BOTH directions.
  ##
  ## `names` are the REAL policy names (spectator side). `alias`, `colour` and
  ## `roles` carry the in-game names. Every score is a mean over the rounds the
  ## episode actually PLAYED: a round the wall clock never reached is excluded
  ## from the mean, not zeroed.
  let
  ...
  var
    ...
    colour = newJArray()
    roles = newJArray()
    roundScores = newJArray()
    bumps = newJArray()
    llmTurns = newJArray()
    fallbackTurns = newJArray()
    coverPct = newJArray()
    tagTicksList = newJArray()
    goalHits = newJArray()
    modes = newJArray()
    roundTicks = newJArray()
    roundEndRules = newJArray()
  for seat in 0 ..< seats:
    let permille = sim.episodePermille(seat)
    names.add(%(
    ...
    win.add(%(not faulted and permille >= 500))
    if seat < sim.players.len:
      alias.add(%(toUpperAscii(teamText(sim.players[seat].team)) & "-" &
        IdentityNames[sim.slotIdentityIndex(seat)]))
      colour.add(%teamText(sim.players[seat].team))
    else:
      alias.add(%(toUpperAscii(teamText(sim.teamForSlot(seat))) & "-" &
        IdentityNames[sim.slotIdentityIndex(seat)]))
      colour.add(%teamText(sim.teamForSlot(seat)))
    var
      seatRoles = newJArray()
      seatRounds = newJArray()
    for entry in sim.roundLog:
      seatRoles.add(%roleName(entry.mode, entry.roles[min(seat, 3)]))
      seatRounds.add(%(entry.permille[min(seat, 3)].float / 1000.0))
    if seatRoles.len == 0:
      ## Never an EMPTY inner array: the results schema bounds them 1..4, and
      ## a fault before the first round banked would otherwise be rejected.
      seatRoles.add(%roleName(sim.mode, sim.roleIndex[min(seat, 3)]))
      seatRounds.add(%0.0)
    roles.add(seatRoles)
  ...
  for entry in sim.roundLog:
    coverPct.add(%entry.coverPct)
    tagTicksList.add(%entry.tagTicks)
    goalHits.add(%entry.goalHits)
    modes.add(%($entry.mode))
    roundTicks.add(%entry.ticks)
    roundEndRules.add(%entry.endRule)
  if modes.len == 0:
    ## The six round-indexed arrays are bounded 1..4 too, so an episode that
    ## faulted before banking anything reports the round it was playing.
    coverPct.add(%0)
    tagTicksList.add(%sim.tagTicks)
    goalHits.add(%0)
    modes.add(%($sim.mode))
    roundTicks.add(%(
      if sim.gameStartTick < 0: 0 else: max(0, sim.tickCount - sim.gameStartTick)))
    roundEndRules.add(%(
      if sim.endRule == EndRuleWallClock: EndRuleWallClock
  ...
  results["roles"] = roles
  results["roundScores"] = roundScores
  results["coverPct"] = coverPct
  results["bumps"] = bumps
  results["tagTicks"] = tagTicksList
  results["goalHits"] = goalHits
  results["llmTurns"] = llmTurns
  results["fallbackTurns"] = fallbackTurns
  results["modes"] = modes
  results["roundTicks"] = roundTicks
  results["roundEndRules"] = roundEndRules
  results["roundsPlayed"] = %sim.roundLog.len
  results["reason"] = %(
    if sim.endReason.len > 0: sim.endReason else: ReasonComplete)
  results["endRule"] = %(
    if sim.endRule.len > 0: sim.endRule else: EndRuleFullTime)
  results["games"] = %max(1, sim.config.maxGames)
  results["finalTick"] = %sim.tickCount
  results["seed"] = %sim.config.seed
  $results

proc mpePlayerResultsJson(sim: SimServer): string =
...
proc playerResultsJson*(sim: SimServer): string =
  ...
  if sim.config.numAgents > 0:
    sim.particleResultsJson()
  else:
    sim.mpePlayerResultsJson()
