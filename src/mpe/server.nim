...
proc httpHandler(request: Request) =
  if request.path == HealthPath and request.httpMethod == "GET":
    ...
    request.respond(200, headers, "MPE server")
  ...
  while true:
        ...
        if not replayLoaded:
          ...
          for websocket, chatText in appState.chatMessages.pairs:
            ...
            if squadMode:
              ...
              # src/particle_worlds_player.nim), so recording every copy would put
    ...
    if squadMode and squadsBuilt and sim.phase == Playing:
      let
        ...
        turnKey = sim.roundIndex * 1_000_000 + turnIndex
      engine.ctl.observeEnemies(sim)
      ## The ROUNDCARD, written once per round on its first turn: the mode, the
      ## four public roles, the goal and its colour, the key and the marks.
      if turnIndex == 0 and turnKey != lastTurnKey:
        replayWriter.writeChat(
          tickTime(sim.tickCount), 0, roundcardRecord(sim))
      if sim.gameTicksElapsed() mod turnTicks == 0 and turnKey != lastTurnKey:
        ...
        for seat in 0 ..< engine.directives.len:
          ...
          ## The SYMBOL install: this seat's one symbol becomes its broadcast
          ## token for the whole 108-tick turn, audible to every seat whatever
          ## the distance (MPE's channel is global — the starter's ShoutRange
          ...
          for order in directive.orders:
            ...
            if order.symbol > 0:
              sim.emitEvent(
                ...
                item = symbolTextOf(order.symbol), amount = turnIndex)
          let record = directive.boundedDirectiveRecord(
            sim.roundIndex + 1, turnIndex, seat, $sim.mode,
            sim.cogAlias(seat), roleName(sim.mode, sim.roleIndex[min(seat, 3)]))
      ...
      # Compile one mask per PARTICLE, in seat order, every tick.
      inputs = newSeq[InputState](sim.players.len)
        ...
        if sim.seatCommands(seat, cogIndex) and seat < engine.directives.len and
          ...
          for candidate in engine.directives[seat].orders:
            if candidate.cogIndex == cogIndex:
              ...
              break
        if not found:
          # This seat has no directive yet (turn 0 has not landed, or the
          # seat never connected). It plays the published `drifter` baseline,
          # so no particle is ever unactuated.
          let scripted = engine.drifterFor(sim, @[cogIndex])
    ...
    if replayLoaded:
      ...
      for _ in 0 ..< playbackSpeed(liveSpeedIndex):
        ...
        if config.maxGames > 0 and phaseBeforeStep != GameOver and
          ...
          inc gamesPlayed
          if squadMode:
            # The round is already banked by checkRoundEnd (or by the
            ...
            if sim.roundLog.len > 0:
              let banked = sim.roundLog[^1]
              echo "round ", gamesPlayed, " done (", $banked.mode,
                ") permille=", banked.permille[0], "/", banked.permille[1],
                "/", banked.permille[2], "/", banked.permille[3]
