...
proc gameDir*(): string =
  ## Returns the MPE game directory.
  getCurrentDir()
...
proc loadCrewSpriteRow*(row: int, label: string): seq[CrewSprite] =
  ...
  if row < 0:
    raise newException(MpeError, "Crew sprite sheet row is negative.")
  let
    path = crewSheetPath()
...
proc soldierRotPixels*(
  ...
  ## One pre-rendered particle sprite (SoldierCanvas·renderScale square,
  ## straight-alpha RGBA): the cog body rotated to aim step `rot`. The master's
  ## FACE side (south) leads the aim — aiming south shows the master exactly as
  ## drawn. The masters are ~120px art rendered down to a 34px body at 1×, so a
  ## renderScale > 1 raster recovers genuine painted detail, not upscaled
  ## blocks. There is NO held weapon: particle worlds has none.
