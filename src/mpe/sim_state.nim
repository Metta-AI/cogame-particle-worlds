...
proc spawnAimBrads*(gameMap: MpeMap, team: Team): int =
...
proc spawnFlipH*(gameMap: MpeMap, team: Team): bool =
...
proc gameHash*(sim: SimServer): uint64 =
  ...
  # --- particle-worlds state, APPENDED after every existing mix so the
  # ordering of the inherited fields stays stable. All of it is gameplay state
  # the wasm viewer re-derives from the recorded masks, so all of it is hashed
  # — EXCEPT commSymbol / commPrev / commTurn, the directive and the note,
  # which are excluded by the starter's rule for anything a commander says.
  result.mixHashInt(sim.roundIndex)
  result.mixHashInt(ord(sim.mode))
  for seat in 0 ..< 4:
    result.mixHashInt(sim.roleIndex[seat])
    result.mixHashInt(sim.perm[seat])
  result.mixHashInt(sim.spawnOffsetBrads)
  result.mixHashInt(sim.goalLandmark)
  for c in 0 ..< 4:
    result.mixHashInt(sim.keySymbols[c])
  result.mixHashInt(sim.landmarks.len)
  for mark in sim.landmarks:
    result.mixHashInt(mark.x)
    result.mixHashInt(mark.y)
    result.mixHashInt(mark.colour)
  for seat in 0 ..< 4:
    result.mixHashInt(sim.bumps[seat])
    result.mixHash(cast[uint64](sim.roundAccum[seat]))
    result.mixHashInt(sim.tagCredit[seat])
    result.mixHashInt(sim.nearestMark[seat])
    result.mixHashInt(sim.settledTicks[seat])
    result.mixHashInt(sim.decodedMark[seat])
    result.mixHashBool(sim.onPointDone[seat])
    result.mixHashBool(sim.tagContact[seat])
  result.mixHash(cast[uint64](sim.coverAccum))
  result.mixHashInt(sim.tagTicks)
  result.mixHashInt(sim.roundsPlayed)
  result.mixHashInt(sim.roundLog.len)
  for entry in sim.roundLog:
    result.mixHashInt(ord(entry.mode))
    result.mixHashInt(entry.ticks)
    result.mixHashInt(entry.goal)
    result.mixHashInt(entry.coverPct)
    result.mixHashInt(entry.tagTicks)
    result.mixHashInt(entry.goalHits)
    for c in entry.endRule:
      result.mixHashInt(ord(c))
    for seat in 0 ..< 4:
      result.mixHashInt(entry.roles[seat])
      result.mixHashInt(entry.permille[seat])
...
proc installSymbol*(sim: var SimServer, seat, symbol, turn: int) =
  ## Installs one seat's broadcast symbol for a turn. NOT HASHED (the
  ## starter's activeDirective rule): nothing a commander says may move the
  ## hash chain, and nothing about the radio touches physics. The viewer
  ## re-derives the radio by re-applying the recorded `directive` records, so
  ## live and replay show the identical strip.
  if seat < 0 or seat >= 4:
    return
  if sim.commTurn[seat] != turn:
    sim.commPrev[seat] = sim.commSymbol[seat]
  sim.commSymbol[seat] = clamp(symbol, 0, SymbolAlphabet.len)
  sim.commTurn[seat] = turn

proc pushFeedDirective*(sim: var SimServer, record: string) =
  ## Records one `directive` chat record for the broadcast feed AND installs
  ## the symbol it carries. Called from the live server as it writes the record
  ## AND from the replay's chat re-application, so the feed and the radio strip
  ## tell the same story either way. Never hashed: this is presentation state.
  ...
  if seat >= 0 and seat < 4:
    let cogs = node{"cogs"}
    if not cogs.isNil and cogs.kind == JArray and cogs.len > 0:
      let text = cogs[0]{"symbol"}.getStr(SymbolSilence)
      var index = 0
      if text.len == 1:
        for i in 0 ..< SymbolAlphabet.len:
          if SymbolAlphabet[i] == text[0]:
            index = i + 1
            break
      sim.installSymbol(seat, index, node{"turn"}.getInt(-1))
