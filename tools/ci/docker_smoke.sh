...
# Substitute: particle-worlds, coworld-particle-worlds, 4.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-particle-worlds:ci)
#   SMOKE_SLUG                 game slug                        (particle-worlds)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/particle-worlds)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/particle-worlds-player)
...
image="${1:-${SMOKE_IMAGE:-coworld-particle-worlds:ci}}"
slug="${SMOKE_SLUG:-particle-worlds}"
...
if [ -n "${ANTHROPIC_API_KEY:-}" ]; then
  player_key_env=(-e "ANTHROPIC_API_KEY=${ANTHROPIC_API_KEY}")
fi
