...
##   header: "MPEFRM01" | u16 slots | u16 mapW | u16 mapH | u16 teams
...
# `key`/`jsonRow` moved to `mpe/events` so the live server and this tool share
...
const
  UsageText =
    "Usage: nim r tools/extract_events.nim [replay-path] [--out <path>] " &
      "[--frames <path>]"
  FramesMagic = "MPEFRM01"
  SeatRecordBytes = 10
  DefaultReplayPath = GameDir / "tests" / "replays" / "mpe.bitreplay"
